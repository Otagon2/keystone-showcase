...
## Round 8 — Org Admin rebuilt to the *real* issue #2 (delegated, 3-skill method)
*Zaki published the full #2 brief (it had been empty; we'd synthesized one and shipped an 8-surface prototype). Delegated the redesign to a background agent running the 3-skill method (design-new research → frontend-design build → ui-ux-audit), then verified independently. The delta was a genuine course-correction, not a fidelity pass.*

- **Surfaces corrected to the brief's five:** **Members · Projects · Labels · Settings · Audit.** Removed **Overview, Secrets, Connections, Billing, API keys** (explicitly out-of-scope in §10 / Part 3 — project-scoped or deferred for v1; only a one-line "Billing/SSO out of scope" note remains in Settings). Added **Labels** (org `key:value` taxonomy: usage counts, rename, **merge**, delete-with-usage-warning, read-only-to-projects note) and **Settings** (sectioned form; email domain **read-only** — "Managed by your Keystone administrator").
- **Roles fixed:** dropped the wrong 4-role Owner/Admin/Editor/Viewer ladder → org roles are `org_admin` | `member` **plus** a separate `can_create_projects` Switch. (Owner/Editor are *project* roles and live elsewhere.)
- **Governance mechanics per appendices C–I:** domain-bound Add-member (Send disabled until `@northwind.com`); Remove **reassigns each owned project** (new-owner Select) *then* type-name-to-confirm; the **at-least-one-admin invariant** blocks demote/remove/self-demote of the sole admin and releases once a second admin exists; **transfer-ownership** (the signature org-only power); Audit is read-only, scope+source badges, before→after, export.
- **Shell corrected:** "Organization" is now reached from the **bottom user-nav cluster** (peer of Inbox / User Settings), org_admin-only + separator-fenced; entering swaps the middle nav to the five surfaces; breadcrumb `Organization · Northwind ▸ {surface}`; a "simulate role" toggle proves the gating (member view hides the entry + default-deny). Seeded the **Priya/Northwind** scenario end-to-end.
- **Verified:** `node --check` PASS; Playwright at localhost — all five surfaces, domain-gate, remove-with-reassignment (writes `project.transferred` ×2 + `member.removed`), last-admin guardrail (blocked → released after promoting a second admin), transfer/archive/label-merge/settings-save/audit-filters+detail+export; **dark + light**; responsive drawer + table scroll < 900px; **0 console errors**. One P2 fixed in audit (long context-note lines capped at 82ch); compact New-York control sizes kept deliberately.
